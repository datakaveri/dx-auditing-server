...
public abstract class AbstractBaseDAO<T extends BaseEntity<T>> implements BaseDAO<T> {
    ...
    public AbstractBaseDAO(PostgresService postgresService, String tableName, String idFileld, Function<JsonObject, T> fromJson) {
    ...
    public Future<T> create(T entity) {
        ...
        return postgresService.insert(query)
                .compose(result -> {
                    ...
                    return Future.succeededFuture(fromJson(result.getRows().getJsonObject(0)));
                })
                .recover(err -> {
    ...
    public Future<T> get(UUID id) {
        Condition condition = new Condition(idFileld, Condition.Operator.EQUALS, List.of(id.toString()));
        ...
        return postgresService.select(query)
                ...
                .recover(err -> {
                    LOGGER.error("Error fetching  from {} ,with ID {}: mesg{}", tableName, id, err.getMessage(), err);
                    return Future.failedFuture(err);
                });
    }
    ...
    public Future<List<T>> getAll() {
        ...
        return postgresService.select(query)
                .compose(result -> {
                    List<T> entities = result.getRows().stream()
                            .map(row -> fromJson((JsonObject) row))
                            .collect(Collectors.toList());
                    return Future.succeededFuture(entities);
                })
                .recover(err -> {
                    LOGGER.error("Error fetching all from: {}, msg: {}", tableName, err.getMessage(), err);
                    return Future.failedFuture(err);
                });
    }
        ...
        return postgresService.delete(query)
                .compose(result -> {
                    if (!result.isRowsAffected()) {
                        return Future.failedFuture("No rows updated when deleting from : " + tableName+ " for id : " + id);
                    }
                    return Future.succeededFuture(true);
                })
                .recover(err -> {
                    LOGGER.error("Error deleting from {} with ID {}: msg{}",tableName, id, err.getMessage(), err);
                    return Future.failedFuture(err);
                });
    }
}
