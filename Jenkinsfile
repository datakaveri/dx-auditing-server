    ...
    stage('Continuous Deployment') {
      ...
      stages {
        ...
        stage('Docker Swarm deployment') {
          steps {
            script {
              sh "ssh azureuser@docker-swarm 'docker service update auditing-tgdex_auditing-tgdex --image ghcr.io/datakaveri/auditing-server-depl:tgdex-5.6.0-${env.GIT_HASH}'"
              sh 'sleep 10'
            }
          }
          post{
